...
import { useIsMobile } from "@areeza/ui/hooks/use-mobile";
...
function SidebarProvider({
  ...
  // Internal open state; openProp/setOpenProp override when controlled.
...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
  ...
  const onMouseDown = React.useCallback(
    (e: React.MouseEvent) => {
      ...
      const sidebarEl = (e.target as HTMLElement).closest(
        "[data-slot='sidebar']",
      );
      const containerEl = sidebarEl?.querySelector(
        "[data-slot='sidebar-container']",
      );
...
function SidebarMenuItem({ className, ...props }: React.ComponentProps<"li">) {
  return (
    <li
      ...
      className={cn("group/menu-item relative select-none", className)}
      {...props}
    />
  );
}
...
const sidebarMenuButtonVariants = cva(
  cn(
    "peer/menu-button group/menu-button flex w-full select-none items-center gap-2 overflow-hidden rounded-md p-2 text-left text-sm ring-sidebar-ring outline-hidden",
    navRowTransition,
    "group-has-data-[sidebar=menu-action]/menu-item:pr-8 group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! focus-visible:ring-2 disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 [&_svg]:size-4 [&_svg]:shrink-0 [&>span:last-child]:truncate",
  ),
  {
    variants: {
      variant: {
        ...
        nav: "text-muted-foreground hover:not-data-active:bg-sidebar-accent/70 hover:not-data-active:text-sidebar-accent-foreground active:not-data-active:bg-sidebar-accent/70 active:not-data-active:text-sidebar-accent-foreground data-active:bg-sidebar-accent data-active:font-medium data-active:text-sidebar-accent-foreground group-data-[proximity]/menu-item:not-data-active:bg-sidebar-accent/50 group-data-[proximity]/menu-item:not-data-active:text-sidebar-accent-foreground",
      },
      size: {
        default: "h-9",
        sm: "h-8 [&_svg]:!size-3.5",
