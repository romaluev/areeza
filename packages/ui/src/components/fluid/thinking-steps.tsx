...
import type { IconName } from "../../lib/ff/icons";
...
/** FF palette names mapped onto Areeza's Badge variants. */
...
const BADGE_COLOR_VARIANT: Record<
  BadgeColor,
  "muted" | "secondary" | "success" | "warn" | "danger"
> = {
  gray: "muted",
  blue: "secondary",
  green: "success",
...
function ThinkingStepSource({ color = "gray", delay = 0, children, className }: ThinkingStepSourceProps) {
  return (
    ...
    >
      <Badge variant={BADGE_COLOR_VARIANT[color]} shape="rounded" className={cn("text-[11px]", className)}>
        {children}
      </Badge>
    </motion.span>
  );
