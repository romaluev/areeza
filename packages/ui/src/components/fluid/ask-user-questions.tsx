...
import { fontWeights } from "../../lib/ff/font-weight";
import { useShape } from "../../lib/ff/shape-context";
import { useIcon } from "../../lib/ff/icons";
import { useProximityHover } from "../../lib/ff/use-proximity-hover";
import { useMergeSplitBlocks, SelectionBackgrounds } from "../../lib/ff/use-merge-split";
...
export interface AskUserQuestionsProps
  ...
  skipLabel?: string;
  /** Hide the "Question X of Y" header row (e.g. for a single question). */
  showHeader?: boolean;
  /** Localize/override the header label. Receives the 0-based index + total. */
  headerLabel?: (index: number, total: number) => ReactNode;
}
...
const AskUserQuestions = forwardRef<HTMLDivElement, AskUserQuestionsProps>(
  function AskUserQuestions(
    ...
    // mouse hover (activeIndex → bg-muted), so keyboard and pointer focus look
          ...
          >
            ...
            >
              ...
              <AnimatePresence>
                {activeRect && (
                  <motion.div
                    ...
                    aria-hidden
                    className={cn(
                      "absolute pointer-events-none bg-muted",
                      shape.bg
                    )}
                    initial={{
                      opacity: 0,
              ...
              {/* Selected-row backgrounds (merged for contiguous selections).
                  ...
                  abutting halves — see useMergeSplitBlocks. Uses bg-accent
              ...
              <AnimatePresence>
                {focusRect && (
                  <motion.div
                    aria-hidden
                    className={cn(
                      "absolute pointer-events-none border border-[var(--ring)] z-20",
                      shape.focusRing
                    )}
                    initial={{
                      opacity: 0,
