// Static lookup tables for surface tokens, so Tailwind's static scanner emits
// the literal `bg-surface-N` / `shadow-surface-N` utilities (Areeza defines
// these in `styles/tokens.css`). Used by the vendored FF Tabs indicator.
